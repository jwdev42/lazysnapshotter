...
class TestBName(unittest.TestCase):
	...
	def test_gt(self):
		base = bnames.BName(2020, 6, 13, 6)
		bigger = [ '2020-06-13.7', '2020-06-14.6', '2020-07-13.6', '2021-06-13.6' ]
		for e in bigger:
			big = bnames.parse(e)
	...
	def test_parse(self):
		malformed = [ '10000-01-01.1', '1956-02-13.0', '42', '666-04-23.1', '2001-01-96.23', '0000-00-00.1', '1999-13-01.2', '1999-12-32.2' ]
		for x in malformed:
			with self.assertRaises(bnames.ParseError):
				bnames.parse(x)
		years = (1, 9, 10, 99, 100, 999, 1000, 9999) #testing all years is too slow
		for y in years:
			months = range(1, 12)
			for m in months:
				days = range(1, 31)
				for d in days:
					...
					for i in indexes:
						str_repr = '{:04d}-{:02d}-{:02d}.{:d}'.format(y, m, d, i)
