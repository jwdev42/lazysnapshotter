...
class Entry:
	...
	flag_unmount: bool = True #true if backup drive should be unmounted after the backup has finished
	...
	backup_dir_relative: Path = None #relative path to the backup drive's snapshot directory starting at the drive's root directory
...
def send_and_receive(source: Path, snapshot_dir: Path, backup_dir: Path):
	...
	src = snapshotkit2.snapshot_dict(snapshotkit2.scan_dir(snapshot_dir, bnames.filter), bnames.parse_path)
	dst = snapshotkit2.snapshot_dict(snapshotkit2.scan_dir(backup_dir, bnames.filter), bnames.parse_path)
	common = snapshotkit2.biggest_common_snapshot(src, dst)
	try:
		trans = Transact(id = sessionkit.session.session_id, source = source, snapshot_dir = snapshot_dir, backup_dir = backup_dir)
		trans.prepare(False)
...
def purge_old_snapshots(directory: Path, keep: int):
	...
	snapshots = snapshotkit2.snapshot_dict(snapshotkit2.scan_dir(directory, bnames.filter), bnames.parse_path)
	keys = list(snapshots)
	if len(keys) > keep:
...
def _create_name(snapshot_dir: Path, backup_dir: Path) -> str:
	...
	snapshot = None
	src = snapshotkit2.snapshot_dict(snapshotkit2.scan_dir(snapshot_dir, bnames.filter_date(datetime.today())), bnames.parse_path)
	dst = snapshotkit2.snapshot_dict(snapshotkit2.scan_dir(backup_dir, bnames.filter_date(datetime.today())), bnames.parse_path)
