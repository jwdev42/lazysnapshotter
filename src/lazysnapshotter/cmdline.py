...
def _parse_snapshots(arg, data):
    ...
    if verify.snapshot_count(ss):
        ...
        raise CommandLineError('Only between 1 and {} snapshots are supported!'.format(
            globalstuff.max_snapshots))
    ...
    while len(args) > 0:
        ...
        if arg == ARG_NOUMOUNT:
            ...
            if ARG_NAME in res.data:
                raise CommandLineError(
                    'The "{}" command supports only one backup name!'.format(ACTION_RUN))
            res.data[ARG_NAME] = arg
        else:
            raise CommandLineError(
                '"{}" is not a valid option for {}'.format(arg, ACTION_RUN))
    if res.data[ARG_NAME] is None:
        raise CommandLineError(
            'The "{}" command needs a backup name!'.format(ACTION_RUN))
...
def _do_modify(res):
    ...
    if not verify.backup_id(backupid):
        raise CommandLineError(
            'Action {}: first argument must be a valid backup name.'.format(ACTION_MODIFY))
