...
_regexes['uuid'] = re.compile(
    '^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$')
_regexes['isodate'] = re.compile(
...
def snapshot_count(c: int):
    if not isinstance(c, int):
        raise TypeError('{}: arg 1 must be of int'.format(
            snapshot_count.__name__))
    ...
    if not path.is_absolute():
        if errmsg is not None:
            ...
            raise VerificationError(
                '"{}" is not an absolute Path!'.format(str(path)))
...
def requireRelativePath(path, errmsg=None):
    ...
    if path.is_absolute():
        if errmsg is not None:
            ...
            raise VerificationError(
                '"{}" is not a relative path!'.format(str(path)))
...
def requireExistingPath(path, errmsg=None):
    ...
    if not path.exists():
        if errmsg is not None:
            ...
            raise VerificationError(
                'Path "{}" does not exist in the file system!'.format(str(path)))
...
def requireRightAmountOfSnapshots(snapshots: int):
    ...
    if not snapshot_count(snapshots):
        raise VerificationError(
            '{} is an invalid amount of snapshots!'.format(snapshots))
